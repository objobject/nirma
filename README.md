...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create nirma-2a1f
  fly apps create nirma-2a1f-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app nirma-2a1f
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app nirma-2a1f-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'nirma-2a1f-staging' does not match app name in config file 'nirma-2a1f'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name nirma-2a1f-db
  fly postgres attach --app nirma-2a1f nirma-2a1f-db

  fly postgres create --name nirma-2a1f-staging-db
  fly postgres attach --app nirma-2a1f-staging nirma-2a1f-staging-db
  ```
